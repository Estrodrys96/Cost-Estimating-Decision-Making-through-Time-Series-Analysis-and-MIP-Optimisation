---
title: "CAAS - Car Purchase Estimating"
...
#setwd("C:/Users/user/Desktop/CAAS")
...
# Load UK postcode lookup table

uk_postcodes_lookup <- read.csv("postcodes.csv")

# Load car data and perform minor formatting
...
# (For all cars) Extract Mode, s.d., and skew parameters of SALE PRICES
```{r}

# Define skewness function
get_skewness_formatted <- function(x) {
  x <- x[!is.na(x)]
  m <- mean(x)
  s <- sd(x)
  n <- length(x)
...
all_cars_formatted$car_price <- as.numeric(all_cars_formatted$car_price)
all_cars_formatted$miles <- as.numeric(all_cars_formatted$miles)

# Get unique years
years_formatted <- sort(unique(all_cars_formatted$year))

# Initialise empty list to store results
results_formatted <- list()

# Loop over years
for (yr in years_formatted) {
  subset_all_cars_formatted <- all_cars_formatted[all_cars_formatted$year == yr, ]
  
  result_row_formatted <- data.frame(
    year = yr,
    
    car_price_median = median(subset_all_cars_formatted$car_price, na.rm = TRUE),
    car_price_sd = sd(subset_all_cars_formatted$car_price, na.rm = TRUE),
    car_price_skew = get_skewness_formatted(subset_all_cars_formatted$car_price),
    
    miles_median = median(subset_all_cars_formatted$miles, na.rm = TRUE),
    miles_sd = sd(subset_all_cars_formatted$miles, na.rm = TRUE),
    miles_skew = get_skewness_formatted(subset_all_cars_formatted$miles)
  )
  
  results_formatted[[length(results_formatted) + 1]] <- result_row_formatted
}

# Combine all rows into a new data frame
all_cars_formatted_sub <- do.call(rbind, results_formatted)

# View the results
print(all_cars_formatted_sub)

# Define the suffixes
fractions_formatted <- c(0.00, 0.25, 0.50, 0.75)

# Repeat each row 4 times
all_cars_formatted_sub_2 <- all_cars_formatted_sub[rep(1:nrow(all_cars_formatted_sub), each = 4), ]

# Reset row names
rownames(all_cars_formatted_sub_2) <- NULL

# Get original years
original_years_formatted <- all_cars_formatted_sub$year

# Create the new year values
new_years_formatted <- as.numeric(rep(original_years_formatted, each = 4)) + rep(fractions_formatted, times = length(original_years_formatted))

# Assign to the data frame
all_cars_formatted_sub_2$year <- new_years_formatted

all_cars_median_sd_skew <- all_cars_formatted_sub_2[!duplicated(all_cars_formatted_sub_2[-1]), ]

# View results
print(all_cars_median_sd_skew)
...
#write.table(combined_data, "C:/Users/user/Desktop/CAAS/audi_mode_forecast.csv", sep = ",", row.names = FALSE)

#write.table(combined_data_sd, "C:/Users/user/Desktop/CAAS/audi_sd_forecast.csv", sep = ",", row.names = FALSE)

#write.table(combined_data_skew, "C:/Users/user/Desktop/CAAS/audi_skew_forecast.csv", sep = ",", row.names = FALSE)
...
#write.table(combined_data_vauxhall, "C:/Users/user/Desktop/CAAS/vauxhall_mode_forecast.csv", sep = ",", row.names = FALSE)

#write.table(combined_data_sd_vauxhall, "C:/Users/user/Desktop/CAAS/vauxhall_sd_forecast.csv", sep = ",", row.names = FALSE)

#write.table(combined_data_skew_vauxhall, "C:/Users/user/Desktop/CAAS/vauxhall_skew_forecast.csv", sep = ",", row.names = FALSE)
...
#write.table(combined_data_volkswagen, "C:/Users/user/Desktop/CAAS/volkswagen_mode_forecast.csv", sep = ",", row.names = FALSE)

#write.table(combined_data_sd_volkswagen, "C:/Users/user/Desktop/CAAS/volkswagen_sd_forecast.csv", sep = ",", row.names = FALSE)

#write.table(combined_data_skew_volkswagen, "C:/Users/user/Desktop/CAAS/volkswagen_skew_forecast.csv", sep = ",", row.names = FALSE)
...
# Output 3D Surface curve for the price-mileage density estimates (for display)
```{r}

# Creates a grid of Z values (density) for X (price) and Y (miles)
vauxhall_density_2d <- kde2d(x = all_cars_vauxhall$car_price,
                    y = all_cars_vauxhall$miles,
                    n = 50) # resolution of the grid

# Create the 3D surface plot
plot_ly(x = vauxhall_density_2d$x, y = vauxhall_density_2d$y, z = vauxhall_density_2d$z, type = "surface") %>%
  layout(title = "Vauxhall Insignia SRi, Density Surface of Car Price vs. Miles",
         scene = list(
           xaxis = list(title = "X - Car Price"),
           yaxis = list(title = "Y - Miles"),
           zaxis = list(title = "Z - Density")
         ))

```

# Mileage to year estimator - Vauxhall
```{r}

# Calculate the median mileage for each year
median_mileage_by_year_vauxhall <- all_cars_vauxhall %>%
  group_by(year) %>%
  summarise(median_mileage = median(miles, na.rm = TRUE)) %>%
  ungroup() %>%
  # Ensure year is numeric for calculations later
  mutate(year = as.numeric(as.character(year)))

print("Median Mileage by Year:")
print(median_mileage_by_year_vauxhall)

# Now, for a given mileage, find the most likely year

given_mileage <- #insert mileage here

# Calculate the absolute difference between given_mileage and each year's median mileage
median_mileage_by_year_vauxhall$difference <- abs(median_mileage_by_year_vauxhall$median_mileage - given_mileage)

# Find the year with the smallest difference
most_likely_year_simple <- median_mileage_by_year_vauxhall %>%
  arrange(difference) %>%
  slice(1) # Get the first row (the one with the smallest difference)

cat(paste0("\nFor a car with ", given_mileage, " miles:\n"))
cat(paste0("The most likely year (based on closest median mileage) is: ", most_likely_year_simple$year, "\n"))
cat(paste0("With a median mileage of: ", most_likely_year_simple$median_mileage, "\n"))
...
# Filter the original data set to only include Vauxhall Insignia SRi data
geog_vauxhall_cars <- all_cars %>%
  filter(make == "Vauxhall", model == "Insignia", variant == "SRi")

# Format location names
geog_vauxhall_cars$car_seller_location <- str_to_title(geog_vauxhall_cars$car_seller_location)

joined_geog_vauxhall <- left_join(geog_vauxhall_cars, uk_postcodes_lookup,
                         by = c("car_seller_location" = "city"))

# Define the list of columns we want to keep
columns_to_keep <- c("X","make", "model", "variant", "car_price",
                     "car_seller_location", "year", "miles",
                     "postcode", "latitude", "longitude", "county")

# The new data frame will contain only the specified columns
cleaned_vauxhall_geog_data <- joined_geog_vauxhall %>%
  dplyr::select(columns_to_keep)


# 1. Define Charing Cross coordinates (Longitude, Latitude)
# Using more precise coordinates from our previous search
charing_cross_lon <- -0.124105728
charing_cross_lat <- 51.50768803
charing_cross_point <- c(charing_cross_lon, charing_cross_lat)


# 2. Prepare the car locations for distHaversine
# distHaversine can take a matrix or data frame of points as its second argument
# Ensure the order is (longitude, latitude) for each row
car_locations_matrix <- as.matrix(cleaned_vauxhall_geog_data[, c("longitude", "latitude")])


# 3. Calculate the distance for every entry
# The result is in meters by default
distances_meters <- distHaversine(charing_cross_point, car_locations_matrix)

# 4. Add the new column "dist_to_london" (in kilometers) to data frame
cleaned_vauxhall_geog_data$dist_to_london_miles <- distances_meters / 1609.34

# Use this frame to calculate transportation costs
Vauxhall_geo_data <- cleaned_vauxhall_geog_data %>%
  dplyr::select(
    1:6, # Keep the first 6 columns as they are
    dist_to_london_miles, # Insert 'dist_to_london_miles' after the 6th column
    everything()          # Place all remaining columns after 'dist_to_london_miles'
  ) %>% 
    dplyr::select(
    1:6, # Keep the first 6 columns as they are
    postcode, # Insert 'dist_to_london_miles' after the 6th column
    everything()  
)
# View the updated data frame to check the new column
# We'll select a few columns to make the output manageable
print(head(Vauxhall_geo_data %>%
             dplyr::select(car_seller_location, latitude, longitude, dist_to_london_km)))



# # Example UK Postcodes
# postcode1 <- "SW1A 0AA" # 10 Downing Street, London
# postcode2 <- "M1 6BD"   # Manchester Arndale, Manchester
# 
# # Look up details for each postcode
# details1 <- postcode_lookup(postcode1)
# details2 <- postcode_lookup(postcode2)
# 
# # Extract Latitude and Longitude
# lat1 <- details1$latitude
# lon1 <- details1$longitude
# 
# lat2 <- details2$latitude
# lon2 <- details2$longitude
# 
# cat(paste0("Coordinates for ", postcode1, ": Lat=", lat1, ", Lon=", lon1, "\n"))
# cat(paste0("Coordinates for ", postcode2, ": Lat=", lat2, ", Lon=", lon2, "\n"))
# 
# 
# 
# 
# # Define the points as longitude, latitude pairs
# # Note: geosphere functions often expect longitude first, then latitude
# charing_cross <- c(51.50806, -0.12803)
# point2 <- c(lon2, lat2)
# 
...
# 
# cat(paste0("\nDistance between ", postcode1, " and ", postcode2, ":\n"))
# cat(paste0("~", round(distance_meters, 2), " meters\n"))
# cat(paste0("~", round(distance_km, 2), " kilometers (as the crow flies)\n"))
...
distance_miles <- c(43, 87, 97, 104, 112, 117, 152, 155, 163, 322, 411)
mean_gbp <- c(107.5, 98.0, 109.0, 117.0, 126.0, 131.5, 171.0, 174.5, 183.5, 362.5, 462.0)

# Data for ranges (min and max)
min_gbp <- c(86, 83, 92, 99, 106, 111, 144, 147, 155, 306, 390)
max_gbp <- c(129, 113, 126, 135, 146, 152, 198, 202, 212, 419, 534)

# Define the model for these two sequences (original, based on index 'x')
x <- 1:length(min_gbp)
min_transport_model <- lm(min_gbp ~ poly(x, 2, raw = TRUE))
max_transport_model <- lm(max_gbp ~ poly(x, 2, raw = TRUE))
mean_transport_model <- lm(mean_gbp ~ poly(x, 2, raw = TRUE))

######

# Create a data frame that includes min and max for the fan plot
cost_data_full <- data.frame(
...
# Build quadratic regression models for mean, min, and max costs using Distance as the predictor
mean_cost_model_dist <- lm(Mean_GBP ~ poly(Distance, 2, raw = TRUE), data = cost_data_full)
min_cost_model_dist <- lm(Min_GBP ~ poly(Distance, 2, raw = TRUE), data = cost_data_full)
max_cost_model_dist <- lm(Max_GBP ~ poly(Distance, 2, raw = TRUE), data = cost_data_full)

# Extract coefficients for all three models
mean_intercept <- coef(mean_cost_model_dist)[1]
mean_coeff_x1 <- coef(mean_cost_model_dist)[2]
mean_coeff_x2 <- coef(mean_cost_model_dist)[3]

min_intercept <- coef(min_cost_model_dist)[1]
min_coeff_x1 <- coef(min_cost_model_dist)[2]
min_coeff_x2 <- coef(min_cost_model_dist)[3]

max_intercept <- coef(max_cost_model_dist)[1]
max_coeff_x1 <- coef(max_cost_model_dist)[2]
max_coeff_x2 <- coef(max_cost_model_dist)[3]

# Create the equation strings for the plot (updated for all three quadratic models)
# Using more precise rounding for coefficients of x and x^2
min_equation_label <- paste0("italic(y)[min] == ", round(min_intercept, 2), " + ", round(min_coeff_x1, 4), " * italic(x) + ", round(min_coeff_x2, 6), " * italic(x)^2")
mean_equation_label <- paste0("italic(y)[mean] == ", round(mean_intercept, 2), " + ", round(mean_coeff_x1, 4), " * italic(x) + ", round(mean_coeff_x2, 6), " * italic(x)^2")
max_equation_label <- paste0("italic(y)[max] == ", round(max_intercept, 2), " + ", round(max_coeff_x1, 4), " * italic(x) + ", round(max_coeff_x2, 6), " * italic(x)^2")

# Determine good coordinates for the equations on the plot to avoid overlap
x_pos <- min(cost_data_full$Distance) + (max(cost_data_full$Distance) - min(cost_data_full$Distance)) * 0.05
y_max_plot_range <- max(cost_data_full$Max_GBP) # Top of plot range
y_min_plot_range <- min(cost_data_full$Min_GBP) # Bottom of plot range
plot_height <- y_max_plot_range - y_min_plot_range

# Position the equations from top to bottom
y_pos_max <- y_max_plot_range - plot_height * 0.05
y_pos_mean <- y_max_plot_range - plot_height * 0.12
y_pos_min <- y_max_plot_range - plot_height * 0.19


# Create the Fan Plot
fan_plot <- ggplot(cost_data_full, aes(x = Distance)) +
  # Add the shaded area for the range (min to max)
  geom_ribbon(aes(ymin = Min_GBP, ymax = Max_GBP), fill = "skyblue", alpha = 0.4) +
  
  # Add the original mean points
  geom_point(aes(y = Mean_GBP), color = "darkblue", size = 3) +
  
  # Add a line connecting the mean points
  geom_line(aes(y = Mean_GBP), color = "darkblue", linetype = "dashed") +
  
  # Add quadratic regression lines for the min, mean, and max
  geom_smooth(aes(y = Min_GBP), method = "lm", formula = y ~ poly(x, 2, raw = TRUE), se = FALSE, color = "orange", linetype = "dotted") + # Quadratic for min
  geom_smooth(aes(y = Mean_GBP), method = "lm", formula = y ~ poly(x, 2, raw = TRUE), se = FALSE, color = "red", linetype = "solid") + # Quadratic for mean
  geom_smooth(aes(y = Max_GBP), method = "lm", formula = y ~ poly(x, 2, raw = TRUE), se = FALSE, color = "orange", linetype = "dotted") + # Quadratic for max
  
  labs(
    title = "Quadratic Regression Model for Transport Cost vs. Distance, \n with Uncertainty Boundaries",
    x = "Distance (miles)",
    y = "Cost (GBP)"
  ) +
  # Add all three equations for the regression lines
  annotate("text", x = x_pos, y = y_pos_max, label = max_equation_label, parse = TRUE, hjust = 0, vjust = 1, size = 4, color = "black") +
  annotate("text", x = x_pos, y = y_pos_mean, label = mean_equation_label, parse = TRUE, hjust = 0, vjust = 1, size = 4, color = "black") +
  annotate("text", x = x_pos, y = y_pos_min, label = min_equation_label, parse = TRUE, hjust = 0, vjust = 1, size = 4, color = "black") +
  
  theme_minimal() +
  scale_color_manual(name = "Data",
                     values = c("Mean Points" = "darkblue", 
                                 "Mean Regression" = "red", 
                                 "Range Boundaries Regression" = "orange")) +
...
ggsave("cost_fan_plot_with_equations.png", plot = fan_plot, width = 10, height = 7, units = "in", dpi = 300)

cat("\nFan plot with all three quadratic equations saved to 'cost_fan_plot_with_equations.png'\n")



min_model_distance <- lm(Min_GBP ~ poly(Distance, 2, raw = TRUE), data = cost_data_full)
summary(min_model_distance)

max_model_distance <- lm(Max_GBP ~ poly(Distance, 2, raw = TRUE), data = cost_data_full)
summary(max_model_distance)
...
# 
# # Original data provided by the transport provider
# distance_miles <- c(43, 87, 97, 104, 112, 117, 152, 155, 163, 322, 411)
# mean_gbp <- c(107.5, 98.0, 109.0, 117.0, 126.0, 131.5, 171.0, 174.5, 183.5, 362.5, 462.0)
# 
# # New data for ranges (min and max)
# min_gbp <- c(86, 83, 92, 99, 106, 111, 144, 147, 155, 306, 390)
# max_gbp <- c(129, 113, 126, 135, 146, 152, 198, 202, 212, 419, 534)
# 
...
#   Distance = distance_miles,
#   Min_GBP = min_gbp,
#   Mode_GBP = mean_gbp, # Assuming mean_gbp is the mode for the triangular distribution
#   Max_GBP = max_gbp
# )
# 
# # Monte Carlo Simulation
# n_simulations <- 10000 # Number of samples for each distance
# 
# # Create an empty list to store simulation results
# simulated_costs_list <- list()
# 
# # Loop through each distance to perform Monte Carlo simulation
# for (i in 1:nrow(cost_parameters)) {
#   dist_val <- cost_parameters$Distance[i]
#   min_val <- cost_parameters$Min_GBP[i]
#   mode_val <- cost_parameters$Mode_GBP[i]
#   max_val <- cost_parameters$Max_GBP[i]
# 
#   # Generate 'n_simulations' random numbers from a triangular distribution
#   # rtriangle(n, a, b, c) -> n=number of samples, a=min, b=max, c=mode
#   simulated_values <- rtriangle(n = n_simulations, a = min_val, b = max_val, c = mode_val)
# 
#   # Store the simulated values along with the distance
#   simulated_costs_list[[i]] <- data.frame(
#     Distance = dist_val,
#     Simulated_Cost = simulated_values
#   )
# }
# 
# # Combine all simulated data into a single data frame
# simulated_costs_df <- bind_rows(simulated_costs_list)
# 
# # Analysis of Simulation Result
# 
# # Calculate summary statistics for simulated costs per distance
# simulation_summary <- simulated_costs_df %>%
#   group_by(Distance) %>%
#   summarise(
#     Simulated_Mean = mean(Simulated_Cost),
#     Simulated_Median = median(Simulated_Cost),
#     Simulated_Min = min(Simulated_Cost),
#     Simulated_Max = max(Simulated_Cost),
#     Simulated_Q25 = quantile(Simulated_Cost, 0.25),
#     Simulated_Q75 = quantile(Simulated_Cost, 0.75),
#     Simulated_SD = sd(Simulated_Cost)
#   )
# 
# cat("Simulation Summary Statistics per Distance\n")
...
# )
# 
# # Plotting the distribution of simulated costs for each distance
# ggplot(simulated_costs_df, aes(x = Simulated_Cost, fill = factor(Distance))) +
#   geom_histogram(binwidth = 5, position = "identity", alpha = 0.6) +
#   facet_wrap(~ Distance, scales = "free_y", ncol = 3) + # Separate plots for each distance
#   labs(
#     title = "Distribution of Simulated Costs per Distance (Triangular Distribution)",
#     x = "Simulated Cost (GBP)",
#     y = "Frequency",
#     fill = "Distance (miles)"
#   ) +
#   theme_minimal() +
#   theme(legend.position = "none") # Hide legend as facet_wrap makes it redundant
# 
# # To save this histogram plot
# # ggsave("simulated_cost_histograms.png", width = 10, height = 8, units = "in", dpi = 300)
# 
# # Plotting Simulated Means vs. Original Means
# ggplot() +
#   geom_point(data = simulation_summary, aes(x = Distance, y = Simulated_Mean, color = "Simulated Mean"), size = 3) +
#   geom_point(data = original_data, aes(x = Distance, y = Mean_GBP, color = "Original Mean"), size = 3, shape = 4, stroke = 1.5) +
#   labs(
#     title = "Comparison of Original Mean vs. Simulated Mean Costs",
#     x = "Distance (miles)",
#     y = "Cost (GBP)",
#     color = "Legend"
#   ) +
#   scale_color_manual(values = c("Simulated Mean" = "blue", "Original Mean" = "red")) +
#   theme_minimal()

# To save this comparison plot
# ggsave("mean_comparison_plot.png", width = 8, height = 6, units = "in", dpi = 300)
...
# Create a 3D plot to choose the best car to buy in terms of price-mileage-distance from London
```{r}
Vauxhall_geo_data <- Vauxhall_geo_data %>% 
 distinct(X, .keep_all = TRUE)

# Calculate Euclidean distance from (0,0,0) for each point 
Vauxhall_geo_data$distance_from_origin <- sqrt(
  Vauxhall_geo_data$miles^2 +
  Vauxhall_geo_data$dist_to_london_miles^2 +
  Vauxhall_geo_data$car_price^2
)

Vauxhall_geo_data <- Vauxhall_geo_data %>%
  dplyr::select(
    1,             # Select the first column (e.g., 'Col_1')
    last_col(),    # Select the last column, placing it second
    everything()   # Select all remaining columns in their original order
)

Vauxhall_geo_data <- Vauxhall_geo_data %>%
  dplyr::select(
    6,             
    all_of("dist_to_london_miles"),     
    everything()   
)
...
closest_point_row <- Vauxhall_geo_data[which.min(Vauxhall_geo_data$distance_from_origin), ]

# Extract the coordinates and ID of the closest point
closest_x_val <- closest_point_row$miles
closest_y_val <- closest_point_row$dist_to_london_miles
closest_z_val <- closest_point_row$car_price
closest_id_val <- closest_point_row$X # Using 'X'

cat(paste0("\n Closest point to (0,0,0) based on Euclidean distance \n"))
cat(paste0("ID: ", closest_id_val, "\n"))
cat(paste0("Miles: ", closest_x_val, "\n"))
cat(paste0("Distance to London: ", closest_y_val, "\n"))
cat(paste0("Car Price: ", closest_z_val, "\n"))
cat(paste0("Euclidean Distance from Origin: ", round(closest_point_row$distance_from_origin, 2), "\n"))


# Create a 3D scatter plot
plot_ly(Vauxhall_geo_data,
        x = ~miles,
        y = ~dist_to_london_miles,
        z = ~car_price,
        type = "scatter3d",
        mode = "markers",
        marker = list(size = 5, color = 'blue'),
        text = ~X, # This provides the text for hoverinfo on the main points
        hoverinfo = 'text') %>%
  add_trace(mode = "text", text = ~X, textfont = list(color = '#000000', size = 10), showlegend = FALSE) %>% # Adds explicit labels
  
  # Add the straight line from (0,0,0) to the closest point
  add_trace(x = c(0, closest_x_val),
            y = c(0, closest_y_val),
            z = c(0, closest_z_val),
            type = "scatter3d",
            mode = "lines",
            line = list(color = 'red', width = 5), # Make the line red and thick
            name = paste0("Line to ", closest_id_val), # Name for the legend
            showlegend = TRUE,
            # Explicitly set text and hoverinfo to NULL/none
            text = NULL,
            hoverinfo = 'none') %>% 
  
  layout(title = "3D Plot of Car Price, Miles, and Distance to London (with Closest Point)",
         scene = list(xaxis = list(title = 'Mileage'),
                      yaxis = list(title = 'Distance to London (ml)'),
                      zaxis = list(title = 'Car Price (GBP)'))
  )
